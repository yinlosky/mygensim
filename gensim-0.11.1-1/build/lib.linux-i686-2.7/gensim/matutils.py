...
class MmWriter(object):
    ...
    def write_meta(self, docno, vector):
        """
        Write a single sparse vector to the file.
        ...
        #vector = sorted((i, w) for i, w in vector if abs(w) > 1e-12) # ignore near-zero entries
        #for termid, weight in vector: # write term ids in sorted order
        self.fout.write(utils.to_utf8("%i %s\n" % (docno + 1, vector))) # +1 because MM format starts counting from 1
        self.last_docno = docno
        return (0, len(vector)) if vector else (-1, 0)
    ...
    def write_corpus(fname, corpus, progress_cnt=1000, index=False, num_terms=None, metadata=False):
        ...
        if hasattr(corpus, 'mymeta'):
            metaw = MmWriter('/run/wiki/results/mymeta.mm')
            metaw.write_headers(-1, -1, -1)
            for docnom, doc in enumerate(corpus):
                if metadata:
                    bow, data = doc
                ...
                if docno % progress_cnt == 0:
                    logger.info("PROGRESS: saving meta #%i" % docno)
                if index:
                    posnowm = metaw.fout.tell()
                    if posnowm == poslastm:
                        offsetsm[-1] = -1
                    offsetsm.append(posnowm)
                    poslastm = posnowm
                max_idm, veclenm = metaw.write_meta(docnom, bow)
                _num_termsm = max(_num_termsm, 1 + max_idm)
                num_nnzm += veclenm

            num_docs = docnom + 1
            num_terms = 1 or _num_termsm
            ...
            metaw.fake_headers(num_docs, num_terms, num_nnz)
